...
class TestPersonalizationService(unittest.TestCase):
    ...
    def test_get_and_score_recommended_articles(
        self, mock_scorer, mock_pref_repo, mock_article_repo
    ):
        mock_article_repo.return_value.get_recommended_articles.return_value = [
            {"id": 1}
        ]
    ...
    def test_score_articles(self, mock_pref_repo, mock_scorer):
        ...
        articles = [{"id": 1}]
        with patch(
            "server.services.personalization_service.ArticleScorer", mock_scorer
        ):
