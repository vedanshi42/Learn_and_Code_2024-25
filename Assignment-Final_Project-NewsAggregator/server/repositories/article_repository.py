...
from server.interfaces.repository_interfaces.i_article_repository import (
    IArticleRepository,
)
...
class ArticleRepository(IArticleRepository):
    ...
    def insert_new_articles(self, articles: list[Article]):
        ...
        for article in clean_articles:
            try:
                ...
                news_agg_logger(
                    20, f"Duplicate article skipped in batch: {article.title}"
                )
                continue
    ...
    def filter_ascii_articles(self, articles: list[Article]):
        return [
            ...
            if article.title
            and article.content
            and self.is_ascii(article.title)
            and self.is_ascii(article.content)
        ]
    ...
    def get_recommended_articles(self, user_id: int):
        try:
            with get_db_cursor() as (cur, db):
                ...
                if not articles:
                    news_agg_logger(
                        30, f"No recommended articles found for user {user_id}."
                    )
        ...
        try:
            with get_db_cursor() as (cur, db):
                ...
                if not user or user["role"] != "admin":
                    news_agg_logger(
                        30,
                        f"Permission denied for user {user_id} to delete article {article_id}.",
                    )
