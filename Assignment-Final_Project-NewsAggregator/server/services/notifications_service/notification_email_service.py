...
class NotificationEmailService:
    ...
    def send_notifications_to_all_users(self):
        try:
            ...
            for user in users:
                try:
                    notifications = self.notifications_repo.get_notifications_for_user(
                        user["user_id"]
                    )
                    ...
                    try:
                        self.emailer.send_email(
                            user["email"], "Your News Notifications", body
                        )
                    except Exception as email_err:
                        news_agg_logger(
                            40, f"Failed to send email to {user['email']}: {email_err}"
                        )
                except Exception as e:
                    news_agg_logger(
                        40,
                        f"Failed to process notifications for user {user['email']}: {e}",
                    )
